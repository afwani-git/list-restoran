...
function loadPlaces(map) {
	...
	_axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
		...
		var marker = places.map(function (place) {
			var _places$location$coor = _slicedToArray(places.location.coordinates, 2),
			    placeLng = _places$location$coor[0],
			    placesLat = _places$location$coor[1];

			var position = { lat: placesLat, lng: placeLng };
...
var TypeAhead = function TypeAhead(search) {
	...
	searchInput.on('input', function () {
		...
		_axios2.default.get('/api/search?q=' + this.value).then(function (res) {
			...
			searchResult.innerHTML = _dompurify2.default.sanitize('<div class="search__result">your search \'' + _this.value + '\' <strong>NOT FOUND</strong></div>');
